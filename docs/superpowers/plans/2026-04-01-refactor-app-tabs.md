# Refactor app.py into Per-Tab Modules Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extract the 4 tabs from `app.py` (2163 lines) into a `tabs/` package, shrinking `app.py` to ~60 lines of setup and delegation.

**Architecture:** Each tab module exposes a `render(ctx: TabContext)` function. `TabContext` is a dataclass carrying shared state (location IDs, display names, location lists). Tab modules read from `ctx` and manage their own `st.session_state` keys.

**Tech Stack:** Python, Streamlit, dataclasses, existing project modules (database, auth, scope, etc.)
...
| File | Responsibility |
|------|---------------|
| `tabs/__init__.py` | Package init, `TabContext` dataclass, re-export `render_*` functions |
| `tabs/upload_tab.py` | Upload zone, detection, smart import, covers sync, delete data, history |
| `tabs/report_tab.py` | Date nav, KPIs, sales/MTD tables, PNG/WhatsApp generation, copy/download |
| `tabs/analytics_tab.py` | Period selector, all 7 chart sections, daily data table |
| `tabs/settings_tab.py` | Account info, outlet CRUD, user CRUD, data export |
| `app.py` | Setup, auth, CSS, tab creation + delegation (shrinks from 2163 → ~60 lines) |

---

### Task 1: Create tabs/ package with TabContext

**Files:**
- Create: `tabs/__init__.py`

- [ ] **Step 1: Create tabs/__init__.py with TabContext dataclass**
...
Run: `python -c "from tabs import TabContext; print(TabContext)"`
Expected: `<class 'tabs.TabContext'>`

- [ ] **Step 3: Commit**

```bash
git add tabs/__init__.py
git commit -m "refactor: create tabs/ package with TabContext dataclass"
```

---

### Task 2: Extract Upload tab into tabs/upload_tab.py

**Files:**
- Create: `tabs/upload_tab.py`
- No other files modified yet

- [ ] **Step 1: Create tabs/upload_tab.py with the full Upload tab code**

Create the file with this content (extracted from `app.py` lines 188–583, adapted to use `ctx`):
...
Run: `python -c "from tabs.upload_tab import render; print('ok')"`
Expected: `ok`

- [ ] **Step 3: Commit**

```bash
git add tabs/upload_tab.py
git commit -m "refactor: extract Upload tab into tabs/upload_tab.py"
```

---

### Task 3: Extract Report tab into tabs/report_tab.py

**Files:**
- Create: `tabs/report_tab.py`

- [ ] **Step 1: Create tabs/report_tab.py with the full Report tab code**

Create the file with this content (extracted from `app.py` lines 585–1081, adapted to use `ctx`):
...
Run: `python -c "from tabs.report_tab import render; print('ok')"`
Expected: `ok`
...
git commit -m "refactor: extract Report tab into tabs/report_tab.py"
```

---

### Task 4: Extract Analytics tab into tabs/analytics_tab.py

**Files:**
- Create: `tabs/analytics_tab.py`

- [ ] **Step 1: Create tabs/analytics_tab.py with the full Analytics tab code**

Create the file with this content (extracted from `app.py` lines 1083–1739, adapted to use `ctx`):
...
def render(ctx: TabContext) -> None:
    ...
    st.caption(
        f"Viewing: **{ctx.report_display_name}** — trends for the period below."
    )
    st.divider()
...
Run: `python -c "from tabs.analytics_tab import render; print('ok')"`
Expected: `ok`
...
git commit -m "refactor: extract Analytics tab into tabs/analytics_tab.py"
```

---

### Task 5: Extract Settings tab into tabs/settings_tab.py

**Files:**
- Create: `tabs/settings_tab.py`

- [ ] **Step 1: Create tabs/settings_tab.py with the full Settings tab code**

Create the file with this content (extracted from `app.py` lines 1741–2163, adapted to use `ctx`):
...
Run: `python -c "from tabs.settings_tab import render; print('ok')"`
Expected: `ok`
...
git commit -m "refactor: extract Settings tab into tabs/settings_tab.py"
```

---

### Task 6: Rewrite app.py to delegate to tab modules

**Files:**
- Modify: `app.py` (entire file replaced)

- [ ] **Step 1: Replace app.py with the slimmed version**
...
Run: `python -c "import app; print('imports ok')"`
Expected: `imports ok` (may show Streamlit warnings but no ImportError)

- [ ] **Step 3: Commit**

```bash
git add app.py
git commit -m "refactor: slim app.py to setup + tab delegation"
```

---

### Task 7: Verify the refactored app works
...
- [ ] **Step 2: Start the Streamlit app to smoke-test**

Run: `streamlit run app.py --server.headless true --server.port 8501`
Expected: App starts without errors on port 8501. Check the output for any import errors or stack traces.

- [ ] **Step 3: Verify line count reduction**

Run: `wc -l app.py tabs/*.py`
Expected: `app.py` ~60 lines, total across tabs ~1650 lines (similar to original 2163 minus the extracted boilerplate)

- [ ] **Step 4: Final commit**

```bash
git add .
git commit -m "refactor: complete app.py tab extraction — all 4 tabs in tabs/ package"
```
...
**Spec coverage check:**
- ✅ `tabs/` package with `TabContext` dataclass → Task 1
- ✅ Upload tab extraction → Task 2
- ✅ Report tab extraction → Task 3
- ✅ Analytics tab extraction → Task 4
- ✅ Settings tab extraction → Task 5
- ✅ `app.py` slimmed to setup + delegation → Task 6
- ✅ Verification/testing → Task 7
- ✅ No behavior changes, pure extraction → All tasks preserve logic

**Placeholder scan:** No TBD/TODO/incomplete sections. All code blocks are complete.

**Type consistency:** `TabContext` fields match across all tasks. All tabs accept `ctx: TabContext`. All `render()` functions return `None`. No signature mismatches.
