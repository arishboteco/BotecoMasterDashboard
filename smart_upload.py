...
class SmartUploadResult:
    ...
    days: List[DayResult]
    customer_content: Optional[bytes]  # raw bytes of customer report (if detected)
...
def process_smart_upload(
    ...
    # Step 5 — group fragments by date and merge
    ...
    for d in sorted(by_date.keys()):
        ...
        if timing_services and not merged.get("services"):
            # Try exact date match first, then fallback to first timing result
            matched_timing = next(
